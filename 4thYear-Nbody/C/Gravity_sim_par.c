               ...
               double G, int N,int thread_N)
{
  ...
  #pragma omp for num_threads(thread_N) collapse(2) private(i,j,x,y,z,r,F,GmM) //schedule(guided,chunk*chunk)
  for(i=0;i<N;i++)
  {
    for(j=0;j<N;j++)
    {
...
void TimeStep(double* position,double* velocity, double* forces,double* masses,
              double dt,int N,int thread_N)
{
  int index,i,k;
  omp_set_num_threads(thread_N);
  #pragma omp parallel for num_threads(thread_N) private(index) collapse(2)
...
int main(int argc, char *argv[])
{
  ...
  //Don't save both
  int save = 0;    //Use this for bigger data sets
  int saveall = 0; //Use this for smaller data sets
  int saveEnergy = 0;

  int N = atoi(argv[1]);                   // Number of particles
  int N_t = atoi(argv[2]);            // Number of time steps
  int t_save = 0;
  ...
  double dt = 1.0/(24*60.0*60.0);
  int i,t,k;
  double stepE;

  int N_t_save = N_t/10000;
  ...
  printf("N_t:%d : thread_N:%d : N:%d\n",N_t,thread_N,N);
  if(save==1)   {position_output = (double*) malloc(3*N*10000*sizeof(double));}
  ...
  ForceFind(position,forces,masses,dt,N,thread_N);
  for(i=0;i<N;i++)
  {
    for(k=0;k<3;k++)
    {
  ...
  for(t=0;t<N_t;t++)
  {
    ...
    ForceFind(position,forces,masses,G,N,thread_N);
    TimeStep(position,velocity,forces,masses,dt,N,thread_N);

  }


  end = omp_get_wtime();
  printf("Time taken %8.6f s\n", end-start);
  ...
  {
    printf("Saving Full\n");
    FILE *f4 = fopen("Gravity_energies_par.csv", "w");
